...
import { SocialPost, SocialGroup, SocialUser, HotZone, SocialComment } from '../types/social';
...
export const SocialService = {
    ...
    async createPost(post: Omit<SocialPost, 'id' | 'timestamp' | 'likes' | 'comments'>): Promise<string> {
        if (!auth.currentUser) throw new Error("Must be logged in");

        // Parse Hashtags
        const hashtags = post.content.match(/#[a-z0-9_]+/gi)?.map(t => t.substring(1).toLowerCase()) || [];
    ...
    async getComments(postId: string): Promise<SocialComment[]> {
        try {
            ...
            const snapshot = await getDocs(q);
            return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as SocialComment));
        } catch (error) {
    ...
    async getGroupsForUser(userId: string): Promise<SocialGroup[]> {
        try {
            ...
            return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), isMember: true } as SocialGroup));
        } catch (error) {
    ...
    async createOrJoinGroup(name: string, interest: string): Promise<SocialGroup> {
        if (!auth.currentUser) throw new Error('Must be logged in');
        ...
        if (!snap.exists()) {
            ...
            return { id: groupId, ...data, isMember: true, memberIds: data.memberIds || [] } as any;
        }
    },
    ...
    async seedDatabase(): Promise<void> {
        ...
        const { MOCK_SOCIAL_FEED, MOCK_GROUPS, MOCK_SOCIAL_USERS, MOCK_HOT_ZONES, MOCK_EVENTS } = await import('../constants');
    ...
    async awardStamp(userId: string, zone: HotZone): Promise<void> {
        ...
        const hasStamp = userData.passportStamps.some(s => s.locationName === zone.name);
    ...
    async wave(sender: SocialUser, receiverId: string): Promise<void> {
        try {
            ...
            if (reverseSnap.exists()) {
                ...
                await setDoc(doc(db, 'notifications', connectNotification.id), connectNotification);
            } else {
                await setDoc(doc(db, COLLECTIONS.WAVES, waveId), {
                    senderId: sender.id,
                    receiverId,
                ...
                const notification: any = { // Using any to avoid strict type checks against UserNotification if imports are tricky, but ideally import it
                    id: `notif_${Date.now()}_${sender.id}`,
                    userId: receiverId,
                    type: 'social',
                    title: 'New Wave! 👋',
    ...
    async getConnectionStatus(userId: string, otherId: string): Promise<'connected' | 'pending' | 'none'> {
