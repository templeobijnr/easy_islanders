      ...
      <div className="space-y-6 animate-in fade-in h-full flex flex-col pb-10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
               ...
               <div>
                  <div className="text-2xl font-bold text-slate-900">{formatMoney(pipelineValue)}</div>
                        ...
                        {filteredClients.map(client => (
                           <tr key={client.id} onClick={() => setSelectedClient(client)} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                              ...
                              <td className="p-4 text-sm text-slate-500 font-mono">
                                 {formatDate((client as any).lastContact, { year: 'numeric', month: 'short', day: 'numeric' })}
                              </td>
                                 ...
                                 >
                                    <div className="flex justify-between items-start mb-2">
                                       <span className="text-xs font-bold text-slate-400 flex items-center gap-1">
                                          <Calendar size={10} /> {formatDate((client as any).lastContact, { year: 'numeric', month: 'short', day: 'numeric' })}
                                       </span>
                                       {client.budget && <span className="text-xs font-bold text-green-600 bg-green-50 px-1.5 py-0.5 rounded">{formatMoney(client.budget)}</span>}
                                    </div>
