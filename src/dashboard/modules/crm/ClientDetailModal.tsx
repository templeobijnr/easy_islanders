               ...
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     ...
                     <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>•</span>
                        <span>Added {formatDate(formData.createdAt || Date.now(), { year: 'numeric', month: 'short', day: 'numeric' })}</span>
                     </div>
                  </div>
               </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div>
                        ...
                        {editMode ? (
                           ...
                           <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase ${formData.status === 'Closed' ? 'bg-green-100 text-green-700' :
                     ...
                     <div className="pt-4 border-t border-slate-100 space-y-4">
                        <div>
                           ...
                           {editMode ? (
                              ...
                              <div className="text-lg font-bold text-slate-900">{formatMoney(formData.budget)}</div>
                           )}
                        </div>
                        <div>
                  ...
                  <div className="flex border-b border-slate-200 bg-white px-6">
                     {['overview', 'activity', 'notes'].map((tab) => (
                        <button
                           ...
                           className={`px-6 py-4 text-sm font-bold capitalize border-b-2 transition-all ${activeTab === tab ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
                              }`}
                        >
                           {tab}
                        </button>
                           ...
                           <div className="space-y-4 relative pl-4 border-l-2 border-slate-200 ml-2">
                              {formData.activityHistory?.map((act) => (
                                 <div key={act.id} className="relative pl-6">
                                    <div className={`absolute -left-[25px] top-0 w-8 h-8 rounded-full border-4 border-slate-50 flex items-center justify-center ${act.type === 'system' ? 'bg-slate-200 text-slate-500' :
