...
import { Loader2, Save, Search, Calendar, Clock, ChevronDown } from "lucide-react";
import ImageUploader from "../../../Shared/ImageUploader";
import LocationPicker from "../../../Shared/LocationPicker";
import { useEventForm } from "./hooks/useEventForm";
import { EVENT_CATEGORIES, REGIONS, REGIONS_WITH_SUBREGIONS } from "./constants";
import type { EventFormProps } from "./types";

const EventForm: React.FC<EventFormProps> = (props) => {
    const vm = useEventForm(props);

    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-900">{props.initialValue ? "Edit Event" : "Add Event"}</h2>
            ...
            <div className="p-6 space-y-6">
                ...
                {vm.showImportPanel && (
                    <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="font-medium text-blue-900">Import Venue from Google</span>
                            <button onClick={() => vm.setShowImportPanel(false)} className="ml-auto text-blue-400 hover:text-blue-600 text-sm">Close</button>
                        </div>
                        <input type="text" placeholder="Search for a venue..." onKeyDown={(e) => e.key === "Enter" && vm.handleImportSearch((e.target as HTMLInputElement).value)} className="w-full px-4 py-2 rounded-lg border border-blue-200 mb-3" />
                ...
                <div>
                    ...
                    <input type="text" value={vm.form.title} onChange={(e) => vm.updateForm({ title: e.target.value })} placeholder="Event name" className="w-full px-4 py-3 rounded-xl border border-slate-200" />
                </div>

                {/* Description */}
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Description</label>
                    <textarea value={vm.form.description} onChange={(e) => vm.updateForm({ description: e.target.value })} placeholder="Describe the event..." rows={4} className="w-full px-4 py-3 rounded-xl border border-slate-200 resize-none" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="date" value={vm.form.eventDate} onChange={(e) => vm.updateForm({ eventDate: e.target.value })} className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200" />
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="time" value={vm.form.eventTime} onChange={(e) => vm.updateForm({ eventTime: e.target.value })} className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200" />
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        <input type="text" value={vm.form.venueName} onChange={(e) => vm.updateForm({ venueName: e.target.value })} placeholder="Venue name" className="flex-1 px-4 py-3 rounded-xl border border-slate-200" />
                        <button onClick={() => vm.setShowImportPanel(true)} className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-sm font-medium hover:bg-blue-100">Import</button>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                        ...
                        <span className="text-slate-700">Enable ticket booking</span>
                    </label>
                </div>
            </div>
