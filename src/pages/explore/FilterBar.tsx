...
import { SlidersHorizontal, MapPin, Building2, BedDouble, Search, Map } from 'lucide-react';
...
const SUB_REGIONS: Record<string, string[]> = {
   'Kyrenia': ['All', 'Central', 'Alsancak', 'Lapta', 'Ozankoy', 'Bellapais', 'Catalkoy', 'Esentepe', 'Karsiyaka', 'Karmi'],
   'Famagusta': ['All', 'Central', 'Salamis', 'Bogaz', 'Kaparaz'],
   'Nicosia': ['All', 'Central', 'Gonyeli', 'Hamitkoy', 'Ortakoy', 'Kaymakli'],
   'Iskele': ['All', 'Long Beach', 'Bafra', 'City Center'],
   'Lefke': ['All', 'Gemikonagi', 'City Center'],
   'Guzelyurt': ['All', 'Kalkanli', 'City Center']
};
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
               ...
               <div className="flex items-center gap-4 w-full md:w-auto">
                  ...
                  <button
                     ...
                     className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 rounded-full text-sm font-bold transition-all ${isFilterOpen
                  ...
                  {activeDomain === 'Real Estate' ? (
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in fade-in slide-in-from-top-2">
                        ...
                        <div className="space-y-4">
                           <div className="space-y-2">
                              ...
                              <select
                                 ...
                                 onChange={(e) => {
                                    ...
                                    onFilterChange('subRegion', 'All'); // Reset sub-region
                                 }}
                           ...
                           {/* Show sub-regions if a specific district is selected */}
                           {filters.location !== 'All' && SUB_REGIONS[filters.location] && (
                              <div className="space-y-2 animate-in fade-in slide-in-from-top-1">
                                 <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Map size={12} /> Area</label>
                                 <select
                                    value={filters.subRegion || 'All'}
                                    onChange={(e) => onFilterChange('subRegion', e.target.value)}
                                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-teal-500"
                                 >
                                    {SUB_REGIONS[filters.location].map(sub => (
                           ...
                           <div className="flex gap-1">
                              {['Any', 'Studio', '1', '2', '3', '4', '5+'].map(b => (
                                 <button
                                    ...
                                    className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-colors ${filters.bedrooms === b
