...
const PaymentCard: React.FC<PaymentCardProps> = ({ booking, onPaid }) => {
  ...
  const paymentBlockReason = !priceValid
    ? "This booking doesn't have a valid price. Please contact support."
    : null;

  const handlePay = async () => {
    // Runtime guard - never trust UI state
    if (!priceValid) {
      setPaymentError("Cannot process payment: price is missing or invalid.");
      return;
    }
    ...
    try {
      ...
      console.error(error);
      setPaymentError("Payment failed to initialize. Please try again.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden my-4">
      ...
      <div className="flex border-b border-slate-100">
        <button
          ...
          className={`flex-1 p-3 text-sm font-bold flex items-center justify-center gap-2 ${method === "card" ? "text-slate-900 border-b-2 border-slate-900" : "text-slate-400"}`}
        >
          <CreditCard size={14} /> Card
        </button>
        <button
          onClick={() => setMethod("crypto")}
          className={`flex-1 p-3 text-sm font-bold flex items-center justify-center gap-2 ${method === "crypto" ? "text-green-600 border-b-2 border-green-600" : "text-slate-400"}`}
        >
      ...
      <div className="p-6">
        ...
        {method === "card" ? (
          ...
          <div className="mb-6 text-center space-y-3">
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 inline-block">
              <div className="w-24 h-24 bg-slate-200 animate-pulse flex items-center justify-center text-xs text-slate-400">
                QR Code
              </div>
            </div>
            <p className="text-[10px] text-green-600 font-bold flex items-center justify-center gap-1">
              <Check size={10} /> TRC20 / ERC20 Supported
            </p>
          </div>
        )}
        ...
        <button
          onClick={handlePay}
          disabled={processing || !!paymentBlockReason}
          className={`w-full py-3 font-bold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50 ${method === "card" ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-green-600 text-white hover:bg-green-700"}`}
        >
          {processing ? (
