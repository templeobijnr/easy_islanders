...
import { MapPin, User, Phone, Navigation, Star, Loader2, Clock } from 'lucide-react';
...
const TaxiStatusCard: React.FC<{ requestId: string }> = ({ requestId }) => {
    ...
    useEffect(() => {
        ...
        console.log(`🚕 [TaxiStatusCard] Listening to request: ${requestId}`);
        ...
        const unsubscribe = onSnapshot(
            ...
            (doc) => {
                if (doc.exists()) {
                    ...
                    console.log(`🚕 [TaxiStatusCard] Request update:`, data.status);
                    setRequest(data);
                } else {
    ...
    if (request.status === 'pending') {
        return (
            <div className="w-full max-w-sm bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border-2 border-amber-200 shadow-lg my-4 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="p-5">
                    <div className="mb-4">
                        <h3 className="font-bold text-lg text-slate-900 mb-1">📡 Broadcasting Request</h3>
                        <p className="text-sm text-slate-600">We've sent your request to nearby drivers</p>
                    </div>
                    ...
                    <div className="space-y-3 mb-4">
                        <div className="flex items-start gap-3 p-3 bg-white rounded-xl border border-amber-100">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-sm font-medium text-slate-900 truncate">{request.pickup.address}</div>
                            </div>
                        </div>
                        <div className="flex items-start gap-3 p-3 bg-white rounded-xl border border-amber-100">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-sm font-medium text-slate-900 truncate">{request.dropoff.address}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 py-3 px-4 bg-white rounded-xl border border-amber-200">
                        ...
                        <span className="text-sm text-slate-600 font-medium">Waiting for driver to accept...</span>
                    </div>
        ...
        <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-lg my-4 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/70 to-transparent">
                    <div className="flex items-center gap-2 text-white">
                        <div className="animate-bounce">
                            <MapPin size={24} className="fill-green-400 text-green-400 drop-shadow-md" />
                        </div>
            ...
            <div className="p-5">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <h3 className="font-bold text-lg text-slate-900">{request.vehicleType || 'Taxi'}</h3>
                ...
                <div className="flex items-center gap-3 mb-4 p-3 bg-slate-50 rounded-xl">
                    ...
                    <div className="flex-1">
                        <div className="font-bold text-sm text-slate-900">{request.driverName || 'Unknown'}</div>
                        <div className="flex items-center gap-1 text-xs text-yellow-500">
                            <Star size={10} className="fill-yellow-500" />
                            {request.rating ? `${request.rating.toFixed(1)} Rating` : 'N/A'}
                        </div>
                    </div>
                    {request.driverPhone && (
                        <a
