...
export function useTaxiRequest(requestId: string | null): UseTaxiRequestResult {
  ...
  useEffect(() => {
    ...
    logger.debug(`🚕 [useTaxiRequest] Setting up listener for: ${requestId}`);
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          // Log status changes
          logger.debug(`🚕 [useTaxiRequest] Status update: ${data.status}`);

          // Optional: Show notifications on status change
          if (data.status === "accepted") {
            logger.debug(`✅ Driver ${data.driverName} accepted your request!`);
          } else if (data.status === "en_route") {
            logger.debug(`🚗 Driver is on the way!`);
          }
        } else {
          setRequest(null);
    ...
    return () => {
      logger.debug(
        `🚕 [useTaxiRequest] Cleaning up listener for: ${requestId}`,
      );
      unsubscribe();
    };
  }, [requestId]);
