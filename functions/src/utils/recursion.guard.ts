 ...
 */
const eventCache = new Map<string, { depth: number; timestamp: number }>();
 ...
 */
export function checkRecursion(ctx: RecursionContext): RecursionCheckResult {
 ...
 */
export function assertRecursionSafe(ctx: RecursionContext): void {
    const result = checkRecursion(ctx);

    if (result.halt) {
        throw new Error(
            `Recursion guard halted execution: ${result.reason}. ` +
 ...
 */
export function withRecursionGuard<T>(
    triggerName: string,
    handler: (
        change: FirebaseFirestore.DocumentSnapshot | FirebaseFirestore.QueryDocumentSnapshot,
        context: { eventId: string; params: Record<string, string> }
    ) => Promise<T>
): (
    change: FirebaseFirestore.DocumentSnapshot | FirebaseFirestore.QueryDocumentSnapshot,
    context: { eventId: string; params: Record<string, string> }
 ...
 */
export function getRecursionCacheSize(): number {
