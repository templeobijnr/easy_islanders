 ...
 */
export const handleVendorReply = async (vendorPhone: string, messageBody: string): Promise<boolean> => {
 ...
 */
async function checkTaxiBookings(vendorPhone: string, normalizedMsg: string, messageBody: string): Promise<boolean> {
    const taxiQuery = await db.collection('taxiBookings')
 ...
 */
async function checkTaxiRequests(vendorPhone: string, normalizedMsg: string, messageBody: string): Promise<boolean> {
    ...
    const taxiRequestQuery = await db.collection('taxiRequests')
    ...
    for (const requestDoc of taxiRequestQuery.docs) {
        ...
        if (!request.broadcastSentTo || !Array.isArray(request.broadcastSentTo)) continue;
 ...
 */
async function checkGroceryOrders(vendorPhone: string, normalizedMsg: string, messageBody: string): Promise<boolean> {
    const groceryQuery = await db.collection('groceryOrders')
 ...
 */
async function checkServiceRequests(vendorPhone: string, normalizedMsg: string, messageBody: string): Promise<boolean> {
    const serviceQuery = await db.collection('serviceRequests')
        .where('status', 'in', ['pending', 'confirmed'])
        .orderBy('createdAt', 'desc')
        .limit(5)
        .get();
...
async function confirmTaxiBooking(bookingId: string, booking: any, vendorMsg: string) {
...
async function rejectTaxiBooking(bookingId: string, booking: any, vendorMsg: string) {
...
async function updateTaxiStatus(bookingId: string, booking: any, status: string, vendorMsg: string) {
...
async function confirmGroceryOrder(orderId: string, order: any, vendorMsg: string) {
...
async function rejectGroceryOrder(orderId: string, order: any, vendorMsg: string) {
...
async function updateGroceryStatus(orderId: string, order: any, status: string, vendorMsg: string) {
...
async function confirmServiceRequest(requestId: string, request: any, vendorMsg: string) {
...
async function rejectServiceRequest(requestId: string, request: any, vendorMsg: string) {
...
async function updateServiceStatus(requestId: string, request: any, status: string, vendorMsg: string) {
...
async function injectSystemMessage(userId: string, content: string) {
    ...
    try {
        ...
        const sessionQuery = await db.collection('chatSessions')
            .where('userId', '==', userId)
            .where('status', '==', 'active')
