...
export interface HeadlessScrapeRequest {
    url: string;
    branchId?: number;
    kind?: string;
...
export type HeadlessResultType =
    | 'headless_json_ok'      // Got JSON with products
    | 'headless_no_items'     // Rendered but no JSON data
    | 'headless_blocked'      // 403/429/503
    | 'headless_captcha'      // Captcha detected
    | 'headless_timeout'      // Timed out
    | 'headless_error';       // Other error

export interface HeadlessExtractionResult {
    type: HeadlessResultType;
    text: string;
    jsonData: any[];
 ...
 */
export async function callHeadlessScraper(request: HeadlessScrapeRequest): Promise<HeadlessExtractionResult> {
    if (!BROWSERLESS_TOKEN) {
        log.warn('[HeadlessScraper] BROWSERLESS_TOKEN not configured');
        return {
            type: 'headless_error',
            text: '',
            jsonData: [],
    ...
    log.info('[HeadlessScraper] Calling Browserless.io', { url });
    ...
    try {
        ...
        if (!response.ok) {
            ...
            log.error('[HeadlessScraper] Browserless error', { status: response.status, text });
        ...
        log.info('[HeadlessScraper] Got HTML response', {
        ...
        if (allProducts.length === 0) {
            ...
            log.info('[HeadlessScraper] Cleaned HTML result', {
        ...
        const textLines = allProducts.slice(0, 100).map((p, i) => {
            ...
            if (p.description || p.content) parts.push(`   ${p.description || p.content}`);
            if (p.price !== undefined) parts.push(`   Price: ${p.currencyType || p.currency || ''} ${p.price}`);
            if (p.categoryName || p.category) parts.push(`   Category: ${p.categoryName || p.category}`);
            return parts.join('\n');
        });

        log.info('[HeadlessScraper] Extracted products', { count: allProducts.length });
        ...
        log.error('[HeadlessScraper] Error', { error: getErrorMessage(e) });

        const timedOut = getErrorMessage(e)?.includes('timeout') || getErrorMessage(e)?.includes('Timeout');
 ...
 */
export function getHeadlessErrorMessage(type: HeadlessResultType): string {
    switch (type) {
