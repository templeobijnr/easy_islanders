...
export const toolResolvers = {
    ...
    requestTaxi: async (args: any, userId?: string) => {
    ...
    dispatchTaxi: async (args: any, userId?: string) => {
        console.log("🚖 Dispatching Taxi:", args);

        try {
            const { sendTaxiRequest } = await import('./twilio.service');
            ...
            if (userId && !customerContact) {
                ...
                if (userSnap.exists) {
                    const userData = userSnap.data();
                    customerContact = (userData as any)?.phone || (userData as any)?.email || '';
                    customerName = (userData as any)?.displayName || customerName;
                }
            }
            ...
            const allTaxisSnap = await db.collection('listings')
                .where('domain', '==', 'Cars')
                .where('type', '==', 'taxi')
                .get();
            ...
            await db.collection('taxiBookings').doc(taxiId).set({
                id: taxiId,
                userId: userId || null,
                customerName,
                customerContact,
    ...
    createPaymentIntent: async (args: any, userId: string) => {
        if (!userId) throw new Error("Unauthorized: User ID required");
    ...
    scheduleViewing: async (args: any, userId: string) => {
        if (!userId) throw new Error("Unauthorized: User ID required");
    ...
    searchLocalPlaces: async (args: any) => {
