...
describe('Knowledge Service', () => {
    describe('chunkText', () => {
        it('should split long text into chunks', () => {
            ...
            const chunks = knowledgeService.chunkText(longText, 500, 100);
        ...
        it('should return single chunk for short text', () => {
            ...
            const chunks = knowledgeService.chunkText(shortText, 500, 100);
...
Contact us at info@restaurant.com for reservations.`;
            ...
            const chunks = knowledgeService.chunkText(text, 150, 30);
        ...
        it('should filter out tiny chunks', () => {
            ...
            const chunks = knowledgeService.chunkText(text, 500, 100);
        ...
        it('should handle empty string', () => {
            const chunks = knowledgeService.chunkText('', 500, 100);
        ...
        it('should preserve content integrity', () => {
            ...
            const chunks = knowledgeService.chunkText(menuText, 200, 50);
    ...
    describe('generateEmbedding', () => {
        ...
        runLiveTests('with live API', () => {
            it('should generate embedding vector', async () => {
                ...
                const embedding = await knowledgeService.generateEmbedding(text);
...
describe('Knowledge Persistence (Integration)', () => {
    ...
    runPersistenceTests('with live Firestore', () => {
        beforeAll(async () => {
            ...
            try {
                await knowledgeService.clearBusinessKnowledge(TEST_BUSINESS_ID);
            } catch {
        ...
        it('should read chunks from Firestore', async () => {
            const chunks = await knowledgeService.listBusinessKnowledge(TEST_BUSINESS_ID);
        ...
        it('should filter by businessId correctly', async () => {
            ...
            const chunks = await knowledgeService.listBusinessKnowledge(WRONG_BUSINESS_ID);

            expect(chunks.length).toBe(0); // Should return nothing for wrong ID
        }, 10000);

        it('should retrieve relevant knowledge for queries', async () => {
            const relevantChunks = await knowledgeService.retrieveKnowledge(
                TEST_BUSINESS_ID,
