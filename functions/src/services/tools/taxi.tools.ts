...
export const taxiTools = {
     ...
     */
    requestTaxi: async (args: RequestTaxiArgs, userId?: string, sessionId?: string): Promise<ToolResult> => {
        ...
        try {
            ...
            if (args.pickupLat && args.pickupLng &&
                (pickupAddress.toLowerCase().includes('current location') || pickupAddress.toLowerCase().includes('see map link'))) {
                console.log(`🗺️ [RequestTaxi] Reverse geocoding pickup: ${args.pickupLat}, ${args.pickupLng}`);
                const geocodedAddress = await reverseGeocode(args.pickupLat, args.pickupLng);
                if (geocodedAddress) {
                    pickupAddress = geocodedAddress;
                    console.log(`✅ [RequestTaxi] Geocoded address: ${geocodedAddress}`);
                }
            }
     ...
     */
    dispatchTaxi: async (args: DispatchTaxiArgs, userId?: string, sessionId?: string): Promise<ToolResult> => {
        ...
        if (locationLower.includes('girne') || locationLower.includes('kyrenia')) {
            district = 'Girne';
        } else if (locationLower.includes('lefkoşa') || locationLower.includes('nicosia') || locationLower.includes('lefkosa')) {
            district = 'Lefkosa';
        } else if (locationLower.includes('magusa') || locationLower.includes('famagusta')) {
            district = 'Famagusta';
        ...
        if (district === 'Unknown' && args.pickupLat && args.pickupLng) {
            district = taxiTools.detectDistrictFromCoordinates(args.pickupLat, args.pickupLng);
            console.log(`🗺️ [DispatchTaxi] Detected district from coordinates: ${district} (${args.pickupLat}, ${args.pickupLng})`);
        }
