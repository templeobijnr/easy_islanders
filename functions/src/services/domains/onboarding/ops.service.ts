...
import { Thread, ThreadMessage, ThreadOnboardingState } from '../../../types/thread.types';
import { ProposalKind } from '../../../types/proposals.types';
import {
    proposalsRepository,
    CreateProposalInput
} from './proposals.repository';
import { updateThreadState } from '../conversations/thread.repository';
import { parseItemsFromText } from './catalog.parser';
import { draftCatalogRepository } from './draftCatalog.repository';
...
export const opsService = {
     ...
     */
    processOpsMessage: async (thread: Thread, message: ThreadMessage): Promise<string> => {
        ...
        if (!state) {
            ...
            // Persist init
            await updateThreadState(thread.id, { onboardingState: state });
        }
        ...
        if (state.pendingProposalId) {
            if (isYes(message.text || '')) {
                // Confirm
                const result = await proposalsRepository.approveAndApply(businessId, state.pendingProposalId);
                if (result.success) {
                    const newState = { ...state };

                    // FSM Transitions on Success
                    if (state.step === 'profile_required') newState.step = 'catalog_required';
                    ...
                    else if (state.step === 'availability_required') newState.step = 'review_and_publish';
                    else if (state.step === 'review_and_publish') newState.step = 'live_ops';

                    newState.pendingProposalId = undefined;
                    newState.lastAppliedProposalId = state.pendingProposalId;

                    await updateThreadState(thread.id, { onboardingState: newState });

                    // Clear draft catalog after successful apply
                    if (state.step === 'catalog_required') {
                        await draftCatalogRepository.clearAll(thread.id);
                    }
                ...
                // Reject
                await proposalsRepository.reject(businessId, state.pendingProposalId);
                await updateThreadState(thread.id, {
                    onboardingState: { ...state, pendingProposalId: undefined }
                });
        ...
        switch (state.step) {
            case 'profile_required': {
                if (!message.text) return "Please send your business name.";

                const patch = { name: message.text, tier: 'prospect' };
                const kind: ProposalKind = 'business_profile_patch';
            ...
            case 'catalog_required': {
                ...
                if (isDone(text)) {
                    ...
                    const draftItems = await draftCatalogRepository.getAll(thread.id);
                    const kind: ProposalKind = 'catalog_upsert';
                ...
                if (isSkip(text)) {
                    ...
                    await draftCatalogRepository.clearAll(thread.id);
                    const newState: ThreadOnboardingState = {
                        ...state,
                        step: 'availability_required',
                        draftCatalogCount: undefined,
                        lastDraftUpdatedAt: undefined,
                ...
                const newCount = await draftCatalogRepository.addItems(thread.id, allNewItems);

                // Update thread pointer
                await updateThreadState(thread.id, {
                    onboardingState: {
                        ...state,
                        draftCatalogCount: Math.min(newCount, MAX_DRAFT_ITEMS),
                        lastDraftUpdatedAt: Timestamp.now(),
                    }
                });

                const itemsDesc = allNewItems.map(i =>
                    'mediaUrl' in i ? '📷 Menu photo' : `${i.name} ${i.currency}${i.price}`
                ).join(', ');
            ...
            case 'availability_required': {
                ...
                let mode: 'always_open' | 'request_based' | 'scheduled' | null = null;
                ...
                const kind: ProposalKind = 'availability_model_set';
...
async function createAndPropose(params: CreateProposalInput & { threadState: ThreadOnboardingState }): Promise<string> {
    const { threadState, ...input } = params;

    // Create Proposal
    const proposal = await proposalsRepository.createIfAbsent(input);

    // Update Thread Pointer
    if (threadState.pendingProposalId !== proposal.id) {
        await updateThreadState(input.threadId, {
            onboardingState: { ...threadState, pendingProposalId: proposal.id }
        });
    }
