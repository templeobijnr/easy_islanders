...
export const broadcastRequest = async (request: TaxiRequest): Promise<void> => {
  ...
  const drivers = await repo.findAvailableTaxis(
    request.pickup.location.district,
  );

  if (drivers.length === 0) {
    logger.debug(
      `No drivers available in district: ${request.pickup.location.district}`,
    );
...
Reply *YES ${requestCode}* to accept.`;
  ...
  logger.debug(
    `Broadcast sent to ${drivers.length} drivers for request ${request.id}`,
  );
};
 ...
 */
export const handleDriverReply = async (
  driverPhone: string,
  messageBody: string,
): Promise<string> => {
  ...
  logger.debug(
    `🔍 [handleDriverReply] Looking for pending request for driver...`,
  );
  const request = await repo.findPendingRequestForDriver(phone);

  if (!request) {
    logger.debug(
      `⚠️ [handleDriverReply] No active requests found for ${phone}`,
    );
...
async function sendChatUpdate(
  ...
  try {
    ...
    if (sessionsSnap.empty) {
      logger.debug(
        "⚠️ [Chat Update] No active session found for user:",
        request.userId,
      );
      return;
    }
    ...
    logger.debug(
      `✅ [Chat Update] Sent taxi status update to session: ${sessionId}`,
    );
  } catch (error) {
...
export const createAndBroadcastRequest = async (
  requestData: Omit<
    TaxiRequest,
    "id" | "status" | "createdAt" | "broadcastSentTo"
  >,
  sessionId?: string,
): Promise<string> => {
  // Create the request
  ...
  if (sessionId) {
    try {
      ...
      logger.debug(
        `✅ [Taxi Service] Stored requestId ${requestId} in session ${sessionId}`,
      );
    } catch (error) {
      console.error(
        `⚠️ [Taxi Service] Failed to store requestId in session:`,
        error,
      );
