...
/**
 * Billing/Payments are explicitly out-of-scope for V1.
 *
 * This suite is intentionally skipped so Stripe configuration (secrets, webhooks)
 * does not block core agent/messaging development.
 *
 * To re-enable:
 * - Remove `.skip` below
 * - Provide `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET`
 */
describe.skip('Payment Service', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    describe('createPaymentIntent', () => {
        ...
        it('should create payment intent for valid booking', async () => {
            ...
            const result = await paymentService.createPaymentIntent(mockBookingId, mockUserId);
        ...
        it('should throw error if booking does not exist', async () => {
            ...
            await expect(paymentService.createPaymentIntent(mockBookingId, mockUserId))
        ...
        it('should throw error if user is not owner', async () => {
            const mockGet = jest.fn().mockResolvedValue({
                exists: true,
                data: () => ({ ...mockBookingData, userId: 'other-user' }),
            });
            const mockDoc = jest.fn().mockReturnValue({ get: mockGet });
            (db.collection as jest.Mock).mockReturnValue({ doc: mockDoc });
    ...
    describe('handleWebhook', () => {
        ...
        it('should process payment_intent.succeeded event', async () => {
            ...
            const result = await paymentService.handleWebhook(mockSignature, mockBody);
        ...
        it('should throw error on invalid signature', async () => {
            ...
            await expect(paymentService.handleWebhook(mockSignature, mockBody))
