...
const generateHotel = (index: number, hotelType: HotelType) => {
  ...
  return {
    ...
    hotelType,
    stars,
    amenities: randomSubset(
      [...realEstateAmenities, "Restaurant", "Bar", "Spa", "Beach Access"],
      5,
      10,
    ),
    ...
    checkInTime: "14:00",
    checkOutTime: "11:00",
    roomTypes: randomSubset(
      ["Standard", "Deluxe", "Suite", "Family Room", "Presidential"],
      2,
      4,
    ),
  };
...
export const populateListings = async () => {
  ...
  try {
    ...
    logger.debug(
      "⚠️  Note: Firestore batch write limit is 500. We need to split into multiple batches...",
    );
    ...
    logger.debug(
      "⚠️  Please use the chunked version to actually write to Firestore",
    );
...
export const populateListingsChunked = async () => {
  ...
  const addToBatch = async (
    ...
    if (batchCount >= chunkSize) {
      logger.debug(
        `📦 Committing batch ${batchNumber} (${batchCount} documents)...`,
      );
  ...
  try {
    ...
    if (batchCount > 0) {
      logger.debug(
        `📦 Committing final batch ${batchNumber} (${batchCount} documents)...`,
      );
      await currentBatch.commit();
