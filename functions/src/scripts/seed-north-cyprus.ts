...
import { City, Place, ServiceProvider } from "../types/v1";
...
async function seedNorthCyprus() {
  ...
  const northCyprusCity: Omit<City, "id"> = {
    name: "North Cyprus",
    country: "Cyprus",
  ...
  const city = await citiesRepository.create(northCyprusCity);
  ...
  const samplePlaces: Omit<Place, "id" | "createdAt" | "updatedAt">[] = [
    // Restaurants
    {
      cityId: city.id,
      ...
      descriptionLong:
        "Famous local pastry shop serving traditional Turkish breakfast, börek, and pastries. A student favorite for affordable and delicious meals.",
      tags: [
        "turkish",
        "breakfast",
        "pastries",
    ...
    },
    {
      ...
      descriptionLong:
        "Iconic harbor-side restaurant offering fresh seafood, meze, and stunning sunset views. Great for special occasions and group dinners.",
    ...
    // Cafes
    {
      ...
      descriptionLong:
        "The go-to coffee spot for EMU students. Offers good WiFi, comfortable seating, and a full menu of coffee drinks and snacks.",
    ...
    // Nightlife
    {
      ...
      descriptionLong:
        "North Cyprus's premier nightlife destination featuring international DJs, live performances, and themed nights. Popular with students and expats.",
    ...
    // Sights
    {
      ...
      descriptionLong:
        "Medieval castle dating back to Byzantine times, housing a famous shipwreck museum. Must-visit historical landmark with stunning harbor views.",
    ...
    },
    {
      ...
      descriptionLong:
        "13th-century Gothic monastery ruins set in the mountains. Offers breathtaking views and hosts occasional concerts. Perfect for a peaceful afternoon.",
    ...
    // Services
    {
      ...
      descriptionLong:
        "Main supermarket for students and residents. Offers groceries, household items, and affordable prices. Multiple locations across North Cyprus.",
  ...
  for (const placeData of samplePlaces) {
    const place = await placesRepository.create(placeData);
  ...
  for (const providerData of sampleProviders) {
    const provider = await serviceProvidersRepository.create(providerData);
