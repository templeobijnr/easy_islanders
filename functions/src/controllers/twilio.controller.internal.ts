 ...
 */
export async function handleHostBookingReply(from: string, body: string): Promise<string | null> {
    ...
    try {
        ...
        const requestQuery = await db.collection('bookingRequests')
        ...
        if (!requestQuery.empty) {
            ...
            const directRef = db.collection('bookings').doc(bookingCode);
        ...
        const guestUserId = booking?.userId || null;
        ...
        // Log communication
        await db.collection('bookingCommunications').add({
