...
const toAbsoluteUrl = (baseUrl: string, relativeUrl?: string): string | null => {
...
const scrapeListing = async (url: string) => {
  ...
  const currency = currencyMeta
    || (priceText?.includes('£') ? 'GBP' : priceText?.includes('$') ? 'USD' : 'GBP');
  ...
  const possibleLocation = $('[class*="location"]').first().text().trim()
    ...
    const validTitle = ogTitle?.trim() && !isProtectionPage('', ogTitle) ? ogTitle.trim() : undefined;
...
export const importPropertyFromUrl = async (req: Request, res: Response) => {
  ...
  try {
        ...
        if (start !== -1 && end !== -1) {
          ...
          if (isValidTitle || isValidPrice || isValidDescription) {
            aiData = {
              ...
              squareMeters: parsed.squareMeters ? Number(parsed.squareMeters) : null,
              plotSize: parsed.plotSize ? Number(parsed.plotSize) : null,
              buildYear: parsed.buildYear ? Number(parsed.buildYear) : new Date().getFullYear(),
              cleaningFee: parsed.cleaningFee ? Number(parsed.cleaningFee) : null,
              monthlyDeposit: parsed.monthlyDeposit ? Number(parsed.monthlyDeposit) : null,
              // Ensure booleans
              depositNeeded: Boolean(parsed.depositNeeded),
              paymentPlanAvailable: Boolean(parsed.paymentPlanAvailable),
              // Ensure arrays
              amenities: Array.isArray(parsed.amenities) ? parsed.amenities : [],
              images: Array.isArray(parsed.images) ? parsed.images.filter((img: string) =>
    ...
    const merged = {
      ...
      amenities: [...(aiData.amenities || []), ...(scrapedAny?.amenities || [])].filter((v, i, a) => a.indexOf(v) === i),
      images: Array.from(new Set([...(aiData.images || []), ...(scraped?.images || [])])).filter(Boolean)
    };
