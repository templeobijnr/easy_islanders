...
const { ragService, retrieveContext, buildSystemPrompt, buildPromptWithContext } = require('./rag.service');
// eslint-disable-next-line @typescript-eslint/no-var-requires
...
describe('RAG Service', () => {
    ...
    describe('retrieveContext', () => {
        it('should return empty result when no chunks found', async () => {
            (knowledgeRepository.findNearestChunks as jest.Mock).mockResolvedValue([]);

            const result = await retrieveContext('business123', 'What are your hours?');
        ...
        it('should apply diversity cap (max 2 chunks per doc)', async () => {
            ...
            (knowledgeRepository.findNearestChunks as jest.Mock).mockResolvedValue(chunks);

            const result = await retrieveContext('business123', 'Tell me about your menu');
        ...
        it('should filter chunks above score threshold', async () => {
            ...
            const result = await retrieveContext('business123', 'Question');
    ...
    describe('buildSystemPrompt', () => {
        it('should include business name', () => {
            const prompt = buildSystemPrompt({ name: 'Awesome Restaurant' });
        ...
        it('should include security rules', () => {
            const prompt = buildSystemPrompt({ name: 'Test Business' });
    ...
    describe('buildPromptWithContext', () => {
        it('should include context when provided', () => {
            ...
            const prompt = buildPromptWithContext(systemPrompt, context, userMessage);
        ...
        it('should handle empty context gracefully with category suggestions', () => {
            ...
            const prompt = buildPromptWithContext(systemPrompt, context, userMessage, { name: 'Test Restaurant', category: 'restaurants' });
    ...
    describe('config', () => {
        it('should expose configuration values', () => {
            expect(ragService.config.TOP_K_RETRIEVE).toBe(20);
            expect(ragService.config.TOP_N_RETURN).toBe(8);
            expect(ragService.config.MAX_CHUNKS_PER_DOC).toBe(2);
            expect(ragService.config.MIN_SCORE_THRESHOLD).toBe(0.7);
        });
    });
});
