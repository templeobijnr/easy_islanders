...
const setAdminRole = async (email) => {
    try {
        ...
        console.log(`Successfully set admin role for user: ${email} (${user.uid})`);
        console.log('Metadata refreshed. Please sign out and sign in again in the frontend to refresh the token.');
    }
    catch (error) {
...
if (!email) {
    ...
    console.log('Usage: npx ts-node -r esbuild-register src/scripts/setAdminRole.ts <email>');
    process.exit(1);
}
setAdminRole(email);
