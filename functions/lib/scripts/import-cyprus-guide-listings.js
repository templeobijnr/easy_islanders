...
async function importListings() {
    try {
        // Read CSV file
        const csvPath = path.join(__dirname, '../../../new_cyprus_guide_listings.csv');
        ...
        console.log(`📊 Found ${records.length} listings to import`);
        // Process in batches
        const batchSize = 500;
        let imported = 0;
        let skipped = 0;
        for (let i = 0; i < records.length; i += batchSize) {
            ...
            for (const record of batchRecords) {
                ...
                if (isNaN(lat) || isNaN(lng)) {
                    console.warn(`⚠️  Skipping ${record.listing_title} - invalid coordinates`);
                    skipped++;
                    continue;
                }
                // Map category
                ...
                const placeData = {
                    ...
                    actions: {
                        taxiEnabled: true,
                        reservationsEnabled: category === 'FOOD' || category === 'NIGHTLIFE' || category === 'CAFE',
                ...
                const docRef = firebase_1.db.collection('places').doc(`cyprus-guide-${record.listing_id}`);
                batch.set(docRef, placeData, { merge: true });
                imported++;
            }
            // Commit batch
            await batch.commit();
            console.log(`✅ Imported batch ${Math.floor(i / batchSize) + 1} (${imported} places so far)`);
        }
        console.log(`\n🎉 Import Complete!`);
...
//# sourceMappingURL=import-cyprus-guide-listings.js.map