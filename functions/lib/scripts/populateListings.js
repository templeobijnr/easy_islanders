...
const generateRealEstateListing = (index, type) => {
    ...
    return {
        ...
        furnishedStatus: randomFromArray(['Unfurnished', 'Semi-Furnished', 'Fully Furnished']),
        titleDeedType: randomFromArray(['Turkish Title', 'Exchange Title', 'TMD Title']),
...
const generateVehicle = (index, type) => {
    ...
    return {
        ...
        fuelType: randomFromArray(['Petrol', 'Diesel', 'Electric', 'Hybrid']),
        seats: randomInt(2, 7),
...
const populateListings = async () => {
    ...
    try {
        ...
        const realEstateTypes = ['short-term', 'long-term', 'sale', 'project'];
        ...
        // Hotels (100 per type)
        const hotelTypes = ['Boutique', 'Resort & Casino', 'City Hotel', 'Bungalow', 'Historic'];
        for (const type of hotelTypes) {
            for (let i = 0; i < 100; i++) {
                const hotel = generateHotel(count, type);
                const docRef = firebase_1.db.collection('listings').doc(`hotel_${type.replace(/\s+/g, '_')}_${i}`);
        ...
        const restaurantTypes = ['Meyhane', 'Seafood', 'Bistro', 'Fine Dining', 'Cafe'];
        for (const type of restaurantTypes) {
            for (let i = 0; i < 100; i++) {
                const restaurant = generateRestaurant(count, type);
                const docRef = firebase_1.db.collection('listings').doc(`restaurant_${type.replace(/\s+/g, '_')}_${i}`);
        ...
        // Events (100 per type)
        const eventTypes = ['Party', 'Concert', 'Cultural', 'Networking', 'Festival'];
        ...
        for (const type of serviceTypes) {
            for (let i = 0; i < 100; i++) {
                ...
                const docRef = firebase_1.db.collection('listings').doc(`service_${type.replace(/\s+/g, '_')}_${i}`);
