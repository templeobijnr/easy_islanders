...
async function main() {
    ...
    try {
        console.log(`🔧 Setting role for UID=${uid} to '${role}'...`);
        const user = await admin.auth().getUser(uid);
        const existingClaims = user.customClaims || {};
        const updatedClaims = Object.assign(Object.assign({}, existingClaims), { role });
        await admin.auth().setCustomUserClaims(uid, updatedClaims);
        console.log('✅ Custom claims updated:', updatedClaims);
        console.log('ℹ️ User must sign out and sign back in for the new role to take effect in ID tokens.');
        process.exit(0);
    }
    catch (err) {
        console.error('❌ Failed to set role:', (err === null || err === void 0 ? void 0 : err.message) || err);
        process.exit(1);
    }
}
