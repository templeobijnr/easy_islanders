...
const handleChatMessage = async (req, res) => {
    ...
    try {
        ...
        const [history, userMemory, userDoc] = await Promise.all([
            ...
            memory_service_1.memoryService.getContext(user.uid),
        ...
        while (functionCalls && functionCalls.length > 0) {
            ...
            for (const call of functionCalls) {
                ...
                if (fnName === 'searchMarketplace') {
                    const items = await toolService_1.toolResolvers.searchMarketplace(Object.assign({}, fnArgs
                    ...
                    toolResult = await toolService_1.toolResolvers.dispatchTaxi(fnArgs);
                }
        ...
        await Promise.all([
            chat_repository_1.chatRepository.saveMessage(sessionId, 'user', [{ text: message }]),
            chat_repository_1.chatRepository.saveMessage(sessionId, 'model', [{ text: text }])
        ]);
        const responseData = {
