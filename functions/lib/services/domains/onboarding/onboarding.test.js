...
jest.mock('firebase-admin/firestore', () => {
    ...
    const getDocsForCollection = (collPath) => {
        ...
        getFirestore: () => ({
                    ...
                    return {
                        ...
                        update: jest.fn().mockImplementation((data) => {
                            if (mockStore.has(docPath)) {
                                mockStore.set(docPath, Object.assign(Object.assign({}, mockStore.get(docPath)), data));
                            }
                        }),
            ...
            runTransaction: jest.fn((callback) => {
                const tx = {
                    ...
                    set: jest.fn().mockImplementation((ref, data, opts) => {
                        const path = ref.path;
                        if ((opts === null || opts === void 0 ? void 0 : opts.merge) && mockStore.has(path)) {
                            mockStore.set(path, Object.assign(Object.assign({}, mockStore.get(path)), data));
                        }
                        else {
                    ...
                    update: jest.fn((ref, data) => {
                        ops.push(() => {
                            if (mockStore.has(ref.path)) {
                                mockStore.set(ref.path, Object.assign(Object.assign({}, mockStore.get(ref.path)), data));
                            }
                        });
                    }),
                    delete: jest.fn((ref) => {
...
describe('Ops Agent & Proposals', () => {
    ...
    const createThread = (onboardingState) => ({
        id: threadId,
        businessId,
        actorId: 'user_123',
    ...
    describe('Ops Service FSM', () => {
        it('should init state to profile_required if missing', async () => {
            ...
            const message = {
                id: 'm_1', text: 'Hi', role: 'user', createdAt: {},
                actorId: 'user_123', channel: 'whatsapp', direction: 'inbound',
                threadId: thread.id,
            };
            await ops_service_1.opsService.processOpsMessage(thread, message);
            expect(thread_repository_1.updateThreadState).toHaveBeenCalledWith(threadId, expect.objectContaining({
        ...
        it('should create proposal when in profile_required and user sends text', async () => {
            ...
            const message = {
                id: 'm_sid', text: 'My Burger Shop', role: 'user', createdAt: {},
                actorId: 'user_123', channel: 'whatsapp', direction: 'inbound',
                threadId: thread.id,
            };
            const response = await ops_service_1.opsService.processOpsMessage(thread, message);
        ...
        it('should approve proposal on YES and advance step', async () => {
            ...
            const message = {
                id: 'm_2', text: 'YES', role: 'user', createdAt: {},
                actorId: 'user_123', channel: 'whatsapp', direction: 'inbound',
                threadId: thread.id,
            };
            const response = await ops_service_1.opsService.processOpsMessage(thread, message);
        ...
        it('should reject proposal on NO and clear pending status', async () => {
            ...
            const message = {
                id: 'm_no', text: 'NO', role: 'user', createdAt: {},
                actorId: 'user_123', channel: 'whatsapp', direction: 'inbound',
                threadId: thread.id,
            };
            const response = await ops_service_1.opsService.processOpsMessage(thread, message);
        ...
        it('should be retry-safe when user sends YES twice (Idempotency)', async () => {
            ...
            const message = {
                id: 'm_yes_retry', text: 'YES', role: 'user', createdAt: {},
                actorId: 'user_123', channel: 'whatsapp', direction: 'inbound',
                threadId: thread.id,
            };
            // First Call
            ...
            thread_repository_1.updateThreadState.mockClear();
            // Second Call (Replay with SAME thread state, simulating race or duplicate delivery before state update persisted/read)
            const response2 = await ops_service_1.opsService.processOpsMessage(thread, message);
    ...
    describe('Proposals Repository', () => {
        it('should be idempotent (return existing proposal)', async () => {
            ...
            // First call
            const p1 = await proposals_repository_1.proposalsRepository.createIfAbsent(input);
            // Second call
            const p2 = await proposals_repository_1.proposalsRepository.createIfAbsent(input);
        ...
        it('should fail apply if proposal not Proposed', async () => {
            ...
            const result = await proposals_repository_1.proposalsRepository.approveAndApply(businessId, proposalId);
