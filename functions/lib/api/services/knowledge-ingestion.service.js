...
function chunkText(text, chunkSize, overlap) {
    ...
    while (start < text.length) {
        ...
        if (end < text.length) {
            ...
            const candidates = [nextPeriod > -1 ? nextPeriod + 1 : text.length, nextNewline > -1 ? nextNewline : text.length];
...
async function extractTextFromPdf(pdfBuffer) {
    ...
    try {
        ...
        if (typeof pageCount === 'number' && pageCount > limits_1.DEFAULT_LIMITS.maxPdfPages) {
        ...
        const charsPerPage = pageCount ? Math.floor(cleaned.length / Math.max(1, pageCount)) : cleaned.length;
    ...
    return { text: normalizeText(result.response.text()), usedVisionFallback: true };
}
async function extractTextFromUrl(url) {
    ...
    try {
        ...
        if (String((error === null || error === void 0 ? void 0 : error.message) || '').startsWith('URL_NOT_ALLOWED')) {
            throw error;
        }
        if (String((error === null || error === void 0 ? void 0 : error.message) || '') === 'URL_TOO_LARGE') {
            throw new Error('URL_NOT_ALLOWED: content too large');
        }
        if (String((error === null || error === void 0 ? void 0 : error.message) || '') === 'DNS_TIMEOUT') {
            throw new Error('URL_NOT_ALLOWED: DNS lookup timed out');
        }
        log_1.log.info('[KnowledgeIngestion] URL extraction failed', { message: error === null || error === void 0 ? void 0 : error.message });
        throw error;
    }
    finally {
...
async function extractTextForDoc(doc) {
    switch (doc.sourceType) {
        ...
        case 'pdf': {
            ...
            return { text: extracted.text, mimeType: 'application/pdf', pageCount: extracted.pageCount };
        }
        case 'file': {
            ...
            if (mimeType.startsWith('image/')) {
                ...
                const extracted = await extractTextFromPdf(file.buffer);
                return { text: extracted.text, mimeType, pageCount: extracted.pageCount };
            }
            else {
                // Treat as text file
...
async function processKnowledgeDocIngestion(payload) {
    ...
    if (doc.status !== 'processing') {
        log_1.log.info('[KnowledgeIngestion] Doc not in processing state, skipping', { status: doc.status });
        return;
    }
    try {
            ...
            try {
                ...
                await knowledge_repository_1.knowledgeRepository.updateDoc(businessId, docId, {
                    ...
                    'catalogExtraction.error': {
                        code: 'EXTRACT_FAILED',
                        message: (extractError === null || extractError === void 0 ? void 0 : extractError.message) || 'Extraction failed'
                    }
                });
            }
        }
        ...
        await knowledge_repository_1.knowledgeRepository.updateDocStatus(businessId, docId, 'failed', {
            code: 'INGEST_FAILED',
            message: (error === null || error === void 0 ? void 0 : error.message) || 'Ingestion failed'
        });
        throw error;
    }
}
