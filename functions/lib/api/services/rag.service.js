 ...
 */
async function retrieveContext(businessId, question) {
    try {
        // 1. Embed the question
        const startEmbed = Date.now();
        const questionVector = await (0, embedding_service_1.embedText)(question);
        const embedLatency = Date.now() - startEmbed;
        log_1.log.info('[RAG] Question embedded', { embedLatencyMs: embedLatency });
        // 2. Query vector search
        const startQuery = Date.now();
        const retrieved = await knowledge_repository_1.knowledgeRepository.findNearestChunks(businessId, questionVector, TOP_K_RETRIEVE);
        const queryLatency = Date.now() - startQuery;
        log_1.log.info('[RAG] Retrieved chunks', { retrievedCount: retrieved.length, queryLatencyMs: queryLatency });
        ...
        log_1.log.info('[RAG] Selected chunks', { selectedCount: selected.length, sourcesCount: sources.length });
        return {
            contextText,
            sources,
        ...
        log_1.log.error('[RAG] retrieveContext failed', error);
        return {
            contextText: '',
            sources: [],
            hasContext: false
 ...
 */
function buildPromptWithContext(systemPrompt, context, userMessage, business) {
    if (!context) {
        // No RAG context - but still be helpful using business info
        const suggestions = getCategorySuggestions(business === null || business === void 0 ? void 0 : business.category);
        return `${systemPrompt}
