...
exports.resolveActorByPhone = resolveActorByPhone;
exports.routeInbound = routeInbound;
 ...
 */
async function resolveActorByPhone(phoneE164) {
    const cleanPhone = phoneE164.replace(/\+/g, '');
    // 1. Check authoritative actors collection (future-proof)
    try {
        const actorDoc = await firebase_1.db.collection('actors').doc(cleanPhone).get();
        if (actorDoc.exists) {
    ...
    const driverQuery = await firebase_1.db.collection('taxi_drivers')
    ...
    const businessQuery = await firebase_1.db.collection('businesses')
        .where('phone', '==', phoneE164)
        .limit(1)
        .get();
 ...
 */
async function routeInbound(payload) {
