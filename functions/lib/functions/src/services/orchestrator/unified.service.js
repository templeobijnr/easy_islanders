...
async function runMerveAgent(thread, inbound, _history) {
    ...
    const { processMessage } = await Promise.resolve().then(() => __importStar(require('../agent/orchestrator.service')));
...
async function runOpsAgent(thread, inbound, _history) {
    ...
    const { opsService } = await Promise.resolve().then(() => __importStar(require('../domains/onboarding/ops.service')));
...
async function processInbound(request) {
    ...
    try {
        ...
        return {
            ...
            error: (0, errors_1.getErrorMessage)(error) || String(error),
        };
    }
}
