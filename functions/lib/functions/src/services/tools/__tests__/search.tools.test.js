...
describe('Search Tools', () => {
    ...
    describe('searchMarketplace', () => {
        it('should return formatted results from Typesense', async () => {
            const results = await search_tools_1.searchTools.searchMarketplace({
                query: 'villa',
                domain: 'Real Estate'
            });
        ...
        it('should pass all filter parameters to Typesense', async () => {
            ...
            await search_tools_1.searchTools.searchMarketplace({
                query: 'apartment',
                domain: 'Real Estate',
                category: 'Apartments',
                subCategory: 'Studio',
        ...
        it('should use wildcard query when no query provided', async () => {
            const { searchListings } = require('../../typesense.service');
            await search_tools_1.searchTools.searchMarketplace({});
            expect(searchListings).toHaveBeenCalledWith(expect.objectContaining({ query: '*' }));
        });
    });
    describe('searchLocalPlaces', () => {
        it('should return formatted results from Mapbox', async () => {
            const results = await search_tools_1.searchTools.searchLocalPlaces({
                query: 'monastery',
                domain: 'Tourism'
            });
            expect(results).toHaveLength(2);
        ...
        it('should combine query and location for better accuracy', async () => {
            ...
            await search_tools_1.searchTools.searchLocalPlaces({
                query: 'restaurant',
                location: 'Kyrenia'
            });
            expect(searchMapboxPlaces).toHaveBeenCalledWith('restaurant in Kyrenia', expect.objectContaining({ types: 'poi', limit: 10 }));
        });
    ...
    describe('searchEvents', () => {
        it('should search for events in the Events domain', async () => {
            const { searchListings } = require('../../typesense.service');
            await search_tools_1.searchTools.searchEvents({
        ...
        it('should return empty array on failure', async () => {
            ...
            const results = await search_tools_1.searchTools.searchEvents({ query: 'test' });
            expect(results).toEqual([]);
        });
    });
    describe('searchHousingListings', () => {
        it('should search with housing-specific filters', async () => {
            const { searchListings } = require('../../typesense.service');
            await search_tools_1.searchTools.searchHousingListings({
                areaName: 'Kyrenia',
                budgetMin: 50000,
                budgetMax: 150000,
                bedrooms: 3
    ...
    describe('searchPlaces', () => {
        it('should try Typesense first for curated places', async () => {
            const { searchListings } = require('../../typesense.service');
            await search_tools_1.searchTools.searchPlaces({ tag: 'beach', category: 'Recreation' }, {});
            expect(searchListings).toHaveBeenCalledWith(expect.objectContaining({
                domain: 'Places',
                query: 'beach',
                category: 'Recreation'
        ...
        it('should fallback to Mapbox when Typesense returns no results', async () => {
            ...
            await search_tools_1.searchTools.searchPlaces({ tag: 'historic', category: 'Tourism' }, {});
        ...
        it('should fallback to Mapbox when Typesense fails', async () => {
            ...
            await search_tools_1.searchTools.searchPlaces({ tag: 'restaurant' }, {});
            expect(searchMapboxPlaces).toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=search.tools.test.js.map