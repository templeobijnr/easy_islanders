...
describe('Taxi Tools', () => {
    ...
    describe('detectDistrictFromCoordinates', () => {
        it('should detect Girne district from valid coordinates', () => {
            const result = taxi_tools_1.taxiTools.detectDistrictFromCoordinates(35.3, 33.3);
        ...
        it.skip('should detect Lefkosa district from valid coordinates', () => {
            const result = taxi_tools_1.taxiTools.detectDistrictFromCoordinates(35.2, 33.4);
            expect(result).toBe('Lefkosa');
        });
        it('should detect Famagusta district from valid coordinates', () => {
            const result = taxi_tools_1.taxiTools.detectDistrictFromCoordinates(35.1, 33.95);
            expect(result).toBe('Famagusta');
        });
        it('should detect Iskele district from valid coordinates', () => {
            const result = taxi_tools_1.taxiTools.detectDistrictFromCoordinates(35.3, 34.0);
            expect(result).toBe('Iskele');
        });
        it('should return Unknown for out-of-bounds coordinates', () => {
            const result = taxi_tools_1.taxiTools.detectDistrictFromCoordinates(40.0, 30.0);
            expect(result).toBe('Unknown');
        });
        it('should return Unknown when coordinates are missing', () => {
            expect(taxi_tools_1.taxiTools.detectDistrictFromCoordinates(undefined, undefined)).toBe('Unknown');
            expect(taxi_tools_1.taxiTools.detectDistrictFromCoordinates(35.3, undefined)).toBe('Unknown');
            expect(taxi_tools_1.taxiTools.detectDistrictFromCoordinates(undefined, 33.3)).toBe('Unknown');
        });
    });
    describe('requestTaxi', () => {
        it('should successfully create a taxi request with valid data', async () => {
            const result = await taxi_tools_1.taxiTools.requestTaxi({
        ...
        it('should handle service errors gracefully', async () => {
            ...
            taxiService.createAndBroadcastRequest.mockRejectedValueOnce(new Error('Database connection failed'));
            const result = await taxi_tools_1.taxiTools.requestTaxi({
                pickupAddress: 'Girne Harbor',
    ...
    describe('dispatchTaxi', () => {
        it('should redirect to requestTaxi with inferred district from location string', async () => {
            const result = await taxi_tools_1.taxiTools.dispatchTaxi({
...
//# sourceMappingURL=taxi.tools.test.js.map