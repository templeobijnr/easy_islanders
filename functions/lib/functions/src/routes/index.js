...
router.post("/listings/:id/messages", auth_1.isAuthenticated, async (req, res) => {
    const { id } = req.params;
    const msg = req.body;
...
router.post("/whatsapp/taxi-request", async (req, res) => {
    try {
        ...
        const resp = await (0, twilio_service_1.sendTaxiRequest)(to, {
            customerContact,
            pickup,
            destination,
            notes,
        ...
        res
            .status(500)
            .json({ error: (0, errors_1.getErrorMessage)(err) || "Failed to send taxi request" });
    }
});
...
router.post("/whatsapp/webhook", async (req, res) => {
    try {
        ...
        if (textLower.includes("taxi") ||
            textLower.includes("driver") ||
            textLower.includes("pickup")) {
            const taxiBookings = await firebase_1.db
            ...
            for (const bookingDoc of taxiBookings.docs) {
                ...
                const isConfirmed = textLower.includes("confirm") ||
                ...
                logger.debug(`✅ Updated taxi booking ${bookingDoc.id} with driver response`);
                ...
                if (booking.userId) {
                    ...
                    if (booking.customerContact) {
                        try {
                            ...
                            logger.debug(`📱 Sent WhatsApp notification to user ${booking.userId}`);
                        }
                        catch (err) {
                            console.error("⚠️ Failed to send WhatsApp to user:", err);
                        }
                    }
                    logger.debug(`🔔 Notified user ${booking.userId} about taxi booking update`);
                }
                break; // Only update the first (most recent) one
            }
        }
        // Handle viewing request responses
        if (textLower.includes("viewing") ||
            textLower.includes("property") ||
            textLower.includes("apartment")) {
            const viewingRequests = await firebase_1.db
            ...
            for (const viewingDoc of viewingRequests.docs) {
                ...
                logger.debug(`✅ Updated viewing request ${viewingDoc.id} with owner response`);
                // Trigger agent to notify the user
                if (viewing.userId) {
                    ...
                    if (viewing.customerContact) {
                        try {
                            ...
                            logger.debug(`📱 Sent WhatsApp notification to user ${viewing.userId}`);
                        }
                        catch (err) {
                            console.error("⚠️ Failed to send WhatsApp to user:", err);
                        }
                    }
                    logger.debug(`🔔 Notified user ${viewing.userId} about viewing request update`);
                }
                break;
            }
        }
