...
function createMockJobDoc(overrides = {}) {
    const now = new Date().toISOString();
    return Object.assign({ id: 'job-test-123', ownerUserId: 'user-123', actionType: 'taxi', actionData: createValidTaxiActionData(), status: 'collecting', jobCode: 'ABC123', language: 'en', createdAt: now, updatedAt: now, dispatchAttempts: 0 }, overrides);
}
// =============================================================================
...
describe('Jobs API - Schema Validation', () => {
    describe('JobSchema', () => {
        it('accepts valid collecting job without merchantTarget', () => {
            ...
            const result = shared_1.JobSchema.safeParse(job);
...
describe('Jobs API - POST /v1/jobs', () => {
    ...
    describe('Idempotency', () => {
        it('returns existing job when clientRequestId matches', async () => {
            ...
            expect(existingJob.clientRequestId).toBe('request-abc-123');
        });
    });
});
// =============================================================================
