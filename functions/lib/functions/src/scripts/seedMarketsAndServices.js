...
exports.seedMarkets = seedMarkets;
...
async function seedMarkets() {
    ...
    for (const market of markets) {
        const docRef = await firebase_1.db.collection("listings").add(market);
...
async function seedServices() {
    ...
    for (const service of services) {
        const docRef = await firebase_1.db.collection("listings").add(service);
...
async function main() {
    ...
    try {
        ...
        logger.debug("\n📝 Summary:");
        logger.debug("   - 3 Markets created (Kyrenia Central, Lapta Express, Bellapais Village)");
        logger.debug("   - 7 Service providers created (Plumber, Electrician, AC, Cleaner, Handyman, Painter, Gardener)");
        ...
        logger.debug("   3. Reply as vendor via WhatsApp to test bi-directional sync");
        process.exit(0);
    }
    catch (error) {
...
//# sourceMappingURL=seedMarketsAndServices.js.map