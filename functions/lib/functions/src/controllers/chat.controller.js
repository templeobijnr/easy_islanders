    ...
    try {
        ...
        if (userLocation) {
            try {
                ...
                console.error("⚠️ [Backend] Failed to persist lastLocation:", (0, errors_1.getErrorMessage)(locErr) || locErr);
            }
        }
                ...
                if (pendingAction.kind !== "confirm_transaction" ||
                    ...
                    if (!confirmResult.success) {
                        if (confirmResult.errorCode === "HOLD_EXPIRED") {
                            ...
                            await chat_repository_1.chatRepository.saveMessage(sessionId, "model", [
                                {
        ...
        `;
        const liteFacts = liteContext.facts && liteContext.facts.length > 0
            ? liteContext.facts.join("; ")
            : "None";
        const liteProbe = ((_a = liteContext.missing) === null || _a === void 0 ? void 0 : _a.length)
        ...
        while (functionCalls && functionCalls.length > 0) {
            ...
            for (const call of functionCalls) {
                ...
                logger.debug(`🛠️ [Backend] Decision: calling tool '${fnName}' with args ${JSON.stringify(fnArgs)}`);
                let toolResult = {};
                try {
                    if (fnName === "searchMarketplace" ||
                        ...
                        if (locationForTaxi &&
                            ...
                            logger.debug(`📍 [Backend] Auto-injected user location into ${fnName}:`, locationForTaxi);
                        }
                        ...
                        const resolver = fnName === "dispatchTaxi"
                        ...
                        if (toolResult.success && toolResult.requestId) {
                            ...
                            logger.debug(`🚕 [Backend] Captured taxi requestId: ${taxiRequestId}`);
                        }
                    }
                    ...
                    const errorMessage = (0, errors_1.getErrorMessage)(toolErr) || "Unknown error occurred";
        ...
        for (const item of allItems) {
            ...
            const searchRecord = {
                ...
                subCategory: itemAny.subCategory ||
                    ...
                    itemAny.type ||
                    (itemAny.domain === "Cars" &&
                        ((_a = itemAny.category) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === "rental"
                        ? "rental"
