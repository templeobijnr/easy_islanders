...
const syncAdminClaims = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: (0, errors_1.getErrorMessage)(error) || "Failed to sync claims" });
    }
};
...
const promoteToAdmin = async (req, res) => {
    try {
        ...
        logger.debug(`[Admin Controller] ${decodedToken.email} promoted ${uid} to admin`);
        res.json({
            status: "success",
        ...
        res.status(500).json({ error: (0, errors_1.getErrorMessage)(error) || "Failed to promote user" });
    }
};
...
const demoteAdmin = async (req, res) => {
    try {
        ...
        logger.debug(`[Admin Controller] ${decodedToken.email} demoted ${targetUid}`);
        res.json({
            status: "success",
        ...
        res.status(500).json({ error: (0, errors_1.getErrorMessage)(error) || "Failed to demote user" });
    }
};
