...
exports.onTaxiStatusChange = (0, firestore_1.onDocumentUpdated)({
    ...
    logger.debug(`🔄 [Taxi Trigger] Status change: ${beforeStatus} → ${afterStatus}`);
    ...
    if (beforeStatus === "pending" && afterStatus === "assigned") {
        ...
        try {
            ...
            if (sessionsSnap.empty) {
                logger.debug(`⚠️ [Taxi Trigger] No active session for user: ${userId}`);
                return;
            }
            ...
            if (pendingRequestId !== requestId) {
                logger.debug(`⚠️ [Taxi Trigger] Session taxi request mismatch: ${pendingRequestId} !== ${requestId}`);
            ...
            logger.debug(`✅ [Taxi Trigger] Agent notified for session: ${sessionId}`);
        }
        catch (error) {
...
async function generateAgentResponse(sessionId, taxiInfo) {
    try {
        logger.debug(`🤖 [Agent Response] Generating response for session: ${sessionId}`);
        // Verify session exists
        const sessionDoc = await firebase_1.db
            .collection("chat_sessions")
            .doc(sessionId)
            .get();
...
Generate a friendly, enthusiastic message to inform the user that their taxi has been confirmed. Include the driver's name and key details. Keep it conversational and reassuring. Do NOT ask any questions - just inform them.`;
        ...
        logger.debug(`💬 [Agent Response] Generated: ${messageText.substring(0, 100)}...`);
