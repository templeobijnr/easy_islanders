...
exports.processSignUp = functions.auth.user().onCreate(async (user) => {
    logger.debug(`[Auth Trigger] New user signed up: ${user.email} (${user.uid})`);
    try {
        ...
        if (user.email && BOOTSTRAP_ADMIN_EMAILS.includes(user.email)) {
            ...
            logger.debug(`[Auth Trigger] Bootstrap: Assigning ADMIN to ${user.email}`);
            ...
            if (userDoc.exists) {
                ...
                if (role === "admin") {
                    ...
                    logger.debug(`[Auth Trigger] Firestore: Assigning ADMIN to ${user.email}`);
                }
                    ...
                    logger.debug(`[Auth Trigger] Firestore: Assigning BUSINESS to ${user.email}`);
                }
                else if (role) {
                    customClaims = {
                        admin: false,
                    ...
                    logger.debug(`[Auth Trigger] Firestore: Assigning ${role} to ${user.email}`);
                }
            }
        }
        ...
        if (customClaims) {
            ...
            logger.debug(`[Auth Trigger] No role determined for ${user.email}, no claims set`);
        }
    }
    catch (error) {
