...
exports.cleanupExpiredCheckIns = functions
    ...
    try {
        const expiredQuery = await firebase_1.db
            .collection("checkins")
            .where("expiresAt", "<", now)
        ...
        await batch.commit();
        logger.debug(`✅ [CheckIn Cleanup] Deleted ${expiredQuery.size} expired check-ins`);
    }
    catch (error) {
...
exports.onCheckInCreate = functions
    ...
    logger.debug(`📍 [CheckIn] User ${data.userId} checked in to ${pinType}/${pinId}`);
...
exports.onCheckInDelete = functions
    ...
    const data = snap.data();
    logger.debug(`📍 [CheckIn] Check-in removed: ${data.userId} from ${data.pinId}`);
});
