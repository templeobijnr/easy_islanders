      ...
      {AVAILABLE_AGENTS.map(agent => {
        ...
        return (
          <button
            ...
            className={`w-full text-left p-3 rounded-xl transition-all duration-200 flex items-start gap-3 ${isActive
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      const aiMsg: Message = {
        ...
        mapLocation: response.mapLocation
      };
          ...
          <div className="md:hidden p-4 border-b border-slate-100 bg-slate-50 flex gap-2 overflow-x-auto scrollbar-hide">
            {AVAILABLE_AGENTS.map(agent => (
              <button
                ...
                className={`flex-shrink-0 px-3 py-2 rounded-full text-xs font-bold flex items-center gap-2 border transition-all ${currentAgent.id === agent.id
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`flex gap-4 max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center shadow-sm ${msg.role === 'user' ? 'bg-slate-900 text-white' : `${currentAgent.color} text-white`
                    }`}>
